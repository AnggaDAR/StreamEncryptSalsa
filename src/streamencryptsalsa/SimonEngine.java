...
public class SimonEngine {
    ...
    public static void encrypt(final int blockSizeBits, final byte[] key, final byte[] io) {
        crypt(true, blockSizeBits, key, io);
    }

    public static void decrypt(final int blockSizeBits, final byte[] key, final byte[] io) {
        crypt(false, blockSizeBits, key, io);
    }

    private static void crypt(final boolean forEncryption, final int blockSizeBits, final byte[] key, final byte[] io) {
     ...
     *
     * @param a {@link KeyParameter} specifying a key with a length appropriate
     * to the configured block size of this engine.
     */
     ...
     *
     * @return the name of the Simon variant, specified to the level of the
     * block size (e.g.
     * <em>Simon96</em>).
     */
    ...
    private static abstract class SimonCipher {
         ...
         * @param sequenceBase the index of the constant sequence to apply to a
         * (hypothetical) 2 word key
         */
         ...
         *
         * @param forEncryption <code>true</code> for encryption,
        ...
        public final void init(boolean forEncryption, byte[] keyBytes) {
            ...
            /* Select constant sequence. Base sequence differs by family, but always increments per key word.*/
        ...
        /**
         * Checks whether a key size provided to the
         * {@link #init(boolean, byte[])} method is valid, and calculates the
         * {@link SimonCipher#rounds} required based on the key length.
         *
        ...
        /**
         * Unpack a block of data into working state prior to an encrypt/decrypt
         * operation.
         *
         ...
         * {@link #unpackBlock(byte[], int)}, leaving the resulting ciphertext
         * words in the working state.
         */
        protected abstract void encryptBlock();

        /**
         * Decrypts the ciphertext words loaded with a previous call to
         * {@link #unpackBlock(byte[], int)}, leaving the resulting plaintext
         * words in the working state.
         */
    ...
    /**
     * Base class of Simon variants that fit in 32 bit Java integers: Simon32,
     * Simon48, Simon64.
     * <p>
     * Simon32 and Simon48 (16 and 24 bit word sizes) are implemented using
     * masking.
     */
    private static abstract class SimonIntCipher
         ...
         * @param sequenceBase the sequence base to select the rounds constants
         * with.
         */
        ...
        protected void setKey(byte[] keyBytes) {
            ...
            for (int i = keyWords; i < rounds; i++) {
                ...
                System.out.println(k[i] + "|" + Integer.toHexString(k[i] & 0xFFFF));
            }
        }
         ...
         * The rotation is on the word size of the cipher instance, not on the
         * full 64 bits of the long.
         *
         * @param i the word to rotate.
         * @param distance the distance in bits to rotate.
         * @return the rotated word, which may have unmasked high (> word size)
         * bits.
         */
        ...
        /**
         * Read {@link SimonCipher#wordSize} bytes from the input data in
         * big-endian order.
         *
         * @param bytes the data to read a word from.
         * @param off the offset to read the word from.
         * @return the read word, with zeroes in any bits higher than the word
         * size.
         */
        ...
        /**
         * Writes {@link SimonCipher#wordSize} bytes into a buffer in big-endian
         * order.
         *
    ...
    /**
     * Base class of Simon variants that fit in 64 bit Java longs: Simon128,
     * Simon96.
     * <p>
